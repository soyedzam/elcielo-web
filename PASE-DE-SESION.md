...
- **"300 lugares" → "Entrada libre" (10-ago-2026).** El congreso pasó de aforo-gatekept a acceso abierto con incentivo de registro: quien se registra antes entra a una **rifa**, quien llega el día del evento sin registro también entra pero sin rifa. El número 300 sigue existiendo como `cupo` en `assets/config.js` (es el tope real del Apps Script, ver §8) pero **ya no se anuncia como "300 lugares" en el copy público** — el hero dice "Entrada: Libre", no un número. No le devuelvas el "Son 300 lugares" a agenda.html/info.html pensando que es información útil: ya no es la promesa que se hace.
...
- **El domingo NO lleva registro y eso es deliberado** (confirmado por Ed, 8-ago-2026). Los otros tres bloques son sesiones del congreso con cupo de 300; el domingo es Culto Único, abierto a toda la congregación y **sin control de acceso**. Por eso su línea dice solo `11:00 — 13:00 aprox.` Si le devuelves un "Registro 10:30" por simetría visual con los otros tres, le estás diciendo a la gente de la iglesia que necesita apartar lugar para entrar a su propio culto.
...
- WhatsApp: `5219991755967` — **solo informes, ya no es el canal de registro** (ver §8)
...
## 8 · Sistema de registro (nuevo, 10-ago-2026) — formulario → Sheet → tablero
...
- **Cross-origin sandboxing:** el tablero corre dentro de un iframe de `script.googleusercontent.com` con sandboxing muy estricto de Google. La automatización de navegador (clicks/tecleo sintético) **no logra escribir en sus campos** — lo intenté 3 formas distintas y todas fallaron en silencio (el campo se queda vacío, sin error). No es un bug del producto: un clic real de una persona sí funciona (Ed lo confirmó). Si necesitas probar el login del tablero, pídele a Ed que lo haga él, o prueba el flujo de datos por otro lado (p.ej. mandar un registro real por el formulario público y verificar la fila en la Sheet directamente).
...
5. ~~Fotos de comunidad~~ — **RESUELTO 10-ago-2026.** Las 3 fotos de "La comunidad que convoca" se reemplazaron por material real de La Nave en silueta/contraluz (nunca rostro reconocible): `007-congregacion-en-adoracion.webp`, `008-la-sala-llena.webp`, `009-luz-de-vela-en-el-culto.webp`. También pasaron a ser el fondo del hero y de la sección de registro (antes estaban en índigo plano).
6. **El hero ahora tiene foto de fondo SIEMPRE visible, no solo el video.** Antes: con `prefers-reduced-motion` activo, el hero se quedaba en gradiente pelado (Ed tiene esa preferencia activada en su iPhone y así lo descubrió). Ahora `007-congregacion-en-adoracion.webp` es el fondo base vía CSS `background`, y el video (cuando autoplay funciona) se reproduce ENCIMA como capa extra — quien pide menos movimiento pierde el video, nunca la imagen. Si tocas `.lx-hero-media` en `styles.css`, no vuelvas a la versión donde el video ES el único fondo.
...
---
*El Cielo en mi Ciudad · Pase de Sesión · v1.2 · 10-ago-2026 · La fragua produce; el taller archiva.* 🕊️
